...
## 7. SP-D2 v1 结果(2026-06-15)
apt `--rooms`(default resolution 1.0):**234 communities**,31.7s。top12 房 59-76 节点,均匀(≈15 节点/房)。
渲染 `out/apt_rooms.png`:多色节点沿骨架,**颜色成局部空间斑块**(Louvain 找到局部连贯簇),但 234 房**严重过分割**(公寓应 5-15 房)。

**判读:全栈(占据→ESDF→GVD→骨架→places 图→rooms)端到端跑通 ✅,机制全对。但 234 房 = 第三次撞同一面墙——上游骨架/图噪声。** SP-A 骨架毛刺 → SP-D 3486 节点 → SP-D2 234 房,每层忠实处理输入、忠实继承噪声。

**关键判断:架构已验证完整,继续往下叠层不会改善质量;杠杆在"清洗",不在"加层"。** 两条根治路:
- **图清洗(廉价)**:剪短死端毛刺 + 降 Louvain resolution(→ 更少更大房)+ 合并 → 房数量级直降。
- **正道 TSDF(根治)**:观测-自由从源头消噪声(要 rosbag)。
