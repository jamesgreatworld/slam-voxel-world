...
**结论**:壳层带是 free-space 的有界化补丁(有效但不够);**干净骨架还缺两步——输入去噪 + GVD 细化/稀疏化**。band 1.0 是当前最优设置,可作粗输入,但不是可视的细骨架。

## 9. SP-A v3 去噪+细化结果(2026-06-15)—— 快轨达成

加 `denoise_occupancy`(连通分量去小碎片)+ `thin_gvd`(skimage 3D skeletonize)。两档并行:

| 配置 | GVD 裁前 | thin 后 | 倍降 |
|---|---|---|---|
| band 1.0 + denoise30 + thin | 1,021,794 | **38,165** | 40× |
| band 1.5 + denoise30 + thin | 1,476,849 | 63,630 | 23× |

截图 `out/apt_gvd_v3a.png`:**清晰的青色细骨架曲线穿过公寓结构**——真正的中轴骨架,不再是云。band 1.0 最干净。

**观察:**
- **thin 是主功臣**(1M→38K,40×);denoise 只丢 1357 个 occ 体素——印证多数噪声与主结构连通,连通分量过滤只能去浮空碎片,大头靠 skeletonize。
- 仍有局部"毛球"缠结(噪声碎片处)+ 大房间贴墙(band 固有),但**全局已能追踪走廊/房间结构**,可视、可作 SP-D 图稀疏化的良好输入。

**SP-A 收口判定:✅ 快轨达成**。settled 配置 = `--band-max 1.0 --min-component 30 --thin`。
干净房间中线(消除毛球+大房间填充)仍需正道(TSDF 观测-自由),但 SP-A 已交付**有界、细化、追踪结构**的骨架,解锁 SP-D。
